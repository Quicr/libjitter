...
std::size_t JitterBuffer::Enqueue(const std::vector<Packet> &packets, const ConcealmentCallback &concealment_callback) {
  ...
  if (gap_to_min.count() > 0) {
    ...
    std::cout << "Below min fill level. Need: " << to_conceal << std::endl;
...
std::size_t JitterBuffer::GenerateConcealment(const std::size_t packets, const ConcealmentCallback& callback) {
  ...
  const std::size_t to_conceal = std::min(packets, space_elements);
  ...
  // std::cout << "Concealment done. Set last written to: " << last_written_sequence_number.value() << std::endl;
...
std::size_t JitterBuffer::Update(const Packet &packet) {
  ...
  // Try and get the first header by moving back elements + metadata.
  const std::size_t this_chunk = latest_written_elements * element_size + METADATA_SIZE;
  if (written_at_start < this_chunk) {
    return 0;
  }
...
std::size_t JitterBuffer::CopyIntoBuffer(const Packet &packet, const bool concealment) {
  ...
  header.concealment = concealment;
